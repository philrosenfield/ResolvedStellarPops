...
def find_photsys_number(photsys,filter):
    mag_file = os.path.join(os.environ['BCDIR'],'tab_mag_odfnew/tab_mag_%s.dat'%photsys)
    mf = open(mag_file,'r')
    nmags = mf.readline()
...
def get_stage_label(region):
    # see parametri.h 
    stage_lab = -1
    if region.upper() == 'PMS': stage_lab = 0  
    if region.upper() == 'MS': stage_lab = 1  
    if region.upper() == 'SUBGIANT': stage_lab = 2  
    if region.upper() == 'RGB': stage_lab = 3  
    if region.upper() == 'HEB': stage_lab = 4
    if region.upper() == 'RHEB': stage_lab = 5
    if region.upper() == 'BHEB': stage_lab = 6
    if region.upper() == 'EAGB': stage_lab = 7
    if region.upper() == 'TPAGB': stage_lab = 8
    if region.upper() == 'POSTAGB': stage_lab = 9
    if region.upper() == 'WD': stage_lab = 10 
    if stage_lab == -1: print '%s not found'%region
    return stage_lab

def get_label_stage(stage_lab):
    # see parametri.h 
    regions = ['PMS','MS','SUBGIANT','RGB','HEB','RHEB','BHEB','EAGB','TPAGB','POSTAGB','WD']
...
def get_loop_data(cmd_input_file,metallicity):
    ...
    file_isotrack = os.path.join(os.environ['ISOTRACK'],filename.replace('isotrack/',''))
    ...
    if not metallicity in z:
        ...
        data = 0
    ptcri = [f[0] for f in files if str(metallicity) in f[0] and f[0].endswith('INT2')][0].replace('isotrack/','')
...
def string_in_lines(lines):
    for line in lines:
        strs = [math_utils.is_numeric(l) for l in line.strip().split() if type(math_utils.is_numeric(l))==str]
        if len(strs)!=0:
            if ' M=' in line:
                print line,strs
...
def read_ptcri(ptcri_file):
    ...
    for line in lines:
        if not 'kind' in line: continue
        try:
            age,logL,logTe,mod,model,cript,M,mass,npt,kind = line.strip().split()
            try:
                float(cript)
            except ValueError:
                mass = float(mass)
                if not mass in d.keys(): d[mass] = {}
                if not cript in d[mass].keys(): d[mass][cript] = {}
    ...
    for j in range(len(mline)-1):
        ...
        for k in range(mline[j]+1,mline[j+1]-1):
            try:
                ...
                if not mass in d.keys(): d[mass] = {}
                if not cript in d[mass].keys(): d[mass][cript] = {}            
                d[mass][cript] = {'age':age,'logTe':logTe,'logL':logL,'model':model}
        if mass == 12: break
    return d    
...
def read_loop_from_ptrci(ptcri_file):
    ...
    d = {}
    for i,line in enumerate(lines):
        if not 'Loop' in line and not 'M=' in line: continue
        if 'M=' in line:
        ...
        if 'Loop' in line:
            ...
            if not mass in d.keys(): d[mass] = {}
            if not loop in d[mass].keys(): d[mass][loop] = {}
            d[mass][loop] = {'age':age,'logTe':logTe,'logL':logL,'model':model}
    return d    
...
def read_isotrack_file(filename):
    if os.path.split(filename)[1].startswith('cmd_input'):
        ...
        filename = os.path.join(os.environ['ISOTRACK'],file_isotrack.replace('isotrack/',''))
    try:
        isotf = open(filename,'r').readlines()
    except:
        isotf = open(os.path.join(os.environ['ISOTRACK'],'parsec',filename),'r').readlines()
    nmets = int(isotf[0])
    z = np.zeros(nmets)
    y = np.zeros(nmets)
    mh = np.zeros(nmets)
...
def write_cmd_input_file(**kwargs):
    ...
    file_tpagb = kwargs.get('file_tpagb','isotrack/tracce_CAF09_AFEP02_I1_S1.dat')
    ...
    fh.write(footer)
    fh.close()
    logger.info('%s wrote %s'%(write_cmd_input_file.__name__,cmd_input_file))
    return cmd_input_file
    
def write_trilegal_input_file(**kwargs):
    default_file = os.path.join(os.environ['TRILEGAL_ROOT'],'input_default.dat')
...
def mc_tests(ID,dir_name,outdir="MC_TESTS",bestfit_loc = "MODELS/"):
    ...
    dummy = [fileIO.ensure_dir(os.path.join(outdir,folder)) for folder in folders]
