...
class CMDregion(object):
    ...
    def __init__(self,region_file):
        data = open(region_file,'r').readlines()
    ...
    def average_color(self,*regs):
        ...
        for reg in regs:
            ...
            mean_colors = [np.mean(rand_col[dinds==i]) for i in range(magbins.size)]
...
def closest_point(point,verts):
    ...
    '''
    return np.argmin(np.sqrt((verts[:,0]-point[0])**2+(verts[:,1]-point[1])**2))
...
def polar_sort(l):
    ...
    slist = sorted(l, key=lambda c: math.atan2(c[0]-mean_col, c[1]-mean_mag), reverse=True)
...
def define_color_mag_region(fitsfiles,region_names,**kwargs):
    ...
    the tagged fitsfile can then be used in match_light, or all other b/r modules that
    make use of MS vs BHeB vs RHeB vs RGB (which isn't picked out well)
    
    writes to two text files:
    1) for each fits file, fitsfile.dat: with ra dec mag1 mag2 mag1err mag2err stage
       stage is set in the code to match trilegal's parametri.h, will be -1 if not 
       assigned
    ...
    if type(fitsfiles) == str: fitsfiles = [fitsfiles]
    for j,fitsfile in enumerate(fitsfiles):
        fileIO.ensure_file(fitsfile)
        datafile = os.path.join(tagged_data_loc,fileIO.replace_ext(fitsfile,'.dat'))
        ...
        outfile = os.path.join(cmd_regions_loc,'regions',\
                     'cmd_regions_%s'%os.path.split(fitsfile)[1].replace('.fits','.dat'))
        out = open(outfile,'a')
        ...
        ax.set_title('')
        plt.draw()
        plt.savefig(os.path.join(plot_dir,os.path.split(datafile)[1].replace('.dat','.png')))
        plt.close()
        
        logger.info('%s wrote %s'%(define_color_mag_region.__name__,datafile.replace('dat','png')))
        fmt='%.8f %.8f %.4f %.4f %.4f %.4f %.0f\n'   
        [dout.write(fmt%(ra[i],dec[i],mag1[i],mag2[i],mag1err[i],mag2err[i],stage[i])) for i in range(len(ra))]
        dout.close()
        logger.info('%s wrote %s'%(define_color_mag_region.__name__,datafile))    
        plt.cla()
        out.close()
